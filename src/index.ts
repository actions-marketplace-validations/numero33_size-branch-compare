...
async function main() {
    ...
    if (eventName === "push") {
        ...
        const sizes = sumSizes(filesSizes);
        debug("Sizes: " + JSON.stringify(sizes, null, 2));

        info("Ref: " + ref);
        fs.writeFileSync(`${sha}-file_sizes.json`, JSON.stringify(filesSizes));
    ...
    debug("pull_requests: " + JSON.stringify(pull_requests));
    if (pull_requests.length > 0) {
        for (const pr of pull_requests) {
            if (pr.base.sha === sha || pr.head.sha === sha) {
                info("UPDATE PR: " + pr.number);

                const baseFiles = await loadCachedFileSizes(client, repo, pr.base.sha);
                debug("baseFiles: " + JSON.stringify(baseFiles, null, 2));
                const headFiles = await loadCachedFileSizes(client, repo, pr.head.sha);
                ...
                const commentBody = [
                    ...
                    markdownTable([
                        ...
                        [
                            "Global",
                            `${signedFixedPercent(differencePercentage(baseSizes.raw, headSizes.raw))} (${prettyBytes(baseSizes.raw - headSizes.raw, {
                                signed: true,
                            })})`,
                            prettyBytes(baseSizes.raw),
                            prettyBytes(headSizes.raw),
                            `${signedFixedPercent(differencePercentage(baseSizes.gzip, headSizes.gzip))} (${prettyBytes(baseSizes.gzip - headSizes.gzip, {
                                signed: true,
                            })})`,
                ...
                if (previousComment) {
                    info("Found previous comment in PR:" + JSON.stringify(previousComment, null, 2));
                    try {
